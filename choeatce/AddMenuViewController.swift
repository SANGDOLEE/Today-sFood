...
class AddMenuViewController: UIViewController {
    ...
    var foodList: [String] = Food.FoodList
...
extension AddMenuViewController: UITableViewDataSource, UITableViewDelegate {
    ...
    func removeFoodFromList(_ food: String) {
        ...
        if !Food.FoodList.contains(food) {
           ...
           // Food.FoodList에서 해당 데이터 삭제
           if let index = Food.FoodList.firstIndex(of: food) {
               Food.FoodList.remove(at: index)
           }
    }
    // 사용자 메뉴 삭제
    func tableView(_ tableView: UITableView, commit editingStyle: UITableViewCell.EditingStyle, forRowAt indexPath: IndexPath) {
        if editingStyle == .delete {
            ...
            // Food.FoodList에서 해당 데이터 삭제
            if let index = Food.FoodList.firstIndex(of: deletedFood) {
                Food.FoodList.remove(at: index)
            }
